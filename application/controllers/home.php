...
public function viewdetail($id){
	$data['set']= $this->produk_model->findId($id);
	$this->template->load('user/wraper','user/isi/viewdetail',$data);
}
   ...
   public function userProfil(){
    ...
    {
    	$data['complete'] = $this->db->get_where('invoice',array('status' => 'complete'));
    	$data['pas'] = $this->db->get_where('invoice',array('status' => 'pending'))->num_rows();
    	$this->template->load('admin/wraper','admin/isi/index',$data);
    }
    ...
    public function procupload(){
 	...
 	function procbayar(){
    	...
    	if (! $this->upload->do_upload('files')) {
    		$error = array('error' => $this->upload->display_errors());
    		redirect('home/kporder/'.$namafile,$error);
    	} else {
    		$statdata= array(
    			'status' => 'pending'
    		);
 	...
 	function procbayaradm(){
    	...
    	if (! $this->upload->do_upload('files')) {
 		...
 		public function addbrand(){
 			$this->template->load('admin/wraper','admin/form/addbrand');
 		}

 		public function addtransaksi(){
 			$this->template->load('admin/wraper','admin/form/addtransaksi');
 		}
    		...
    		if (! $this->upload->do_upload('poto')) {
 			...
 			case 'brand':
 				$data = array(
    				...
    				'manufacture' => $this->input->post('mnfc'),
    				'alamat' => $this->input->post('alamat'),
    				'email' => $this->input->post('email')
    				
    			);

 				if ($this->produk_model->insertUser('brand',$data)) {
    				$notif = "<div class='alert bg-success'><i class='fa fa-alert'></i>Tambah Data berhasil</div>";
 					$this->session->set_flashdata('notif',$notif);
    				redirect('home/showbrand');
    			}
 				break;
 		}
 		...
 		public function editkategori(){
 			$id = $this->uri->segment(3);
 			$idp = array('id_kategori' => $id);
 			$data['where'] = $this->produk_model->getdataw($idp,'kategori')->row();
 			// echo "<pre>";
 			// print_r($data);
 			// echo "</pre>";
 			$this->template->load('admin/wraper','admin/form/editkategori',$data);
 		}


 		public function editbrand(){
 			$id = $this->uri->segment(3);
 			$idp = array('id_brand' => $id);
 			$data['where'] = $this->produk_model->getdataw($idp,'brand')->row();
 			// echo "<pre>";
 			// print_r($data);
 			// echo "</pre>";
 			$this->template->load('admin/wraper','admin/form/editbrand',$data);
 		}

 		public function edittrans(){
 			$id = $this->uri->segment(3);
 			$idp = array('id_transaksi' => $id);
 			$data['idvc'] = $this->produk_model->getdataw($idp,'transaksi')->row();
 			// echo "<pre>";
 			// print_r($data);
 			// echo "</pre>";
 			$this->template->load('admin/wraper','admin/form/edittrans',$data);
 		}
 			...
 			switch ($seleksi) {
 				...
 				case 'kategori':
    				...
    				$data = array(
    					...
    					if ($this->produk_model->editdata('kategori',$data,$where)) {
 				...
 				case 'brand':
    				...
    				$data = array(
    					...
    					if ($this->produk_model->editdata('brand',$data,$where)) {
 				...
 				case 'transaksi':
    				...
    				'id_invoice' => $this->input->post('ivc'),
    				'total_bayar' => $this->input->post('tobar'),
    				'nama_bank' => $this->input->post('cekbank'),
    				'no_rekening' => $this->input->post('norek'),
    				'bank_to' => $this->input->post('bankto'),
 		...
 		public function delkategori(){
 			...
 			if ($this->produk_model->deldata('kategori',$where)) {
 				$notif = "<div class='alert bg-success'><i class='fa fa-alert'></i>Data berhasil di hapus</div>";
 				$this->session->set_flashdata('notif',$notif);
 				redirect('home/showkategori');
 			}
 		...
 		public function delbrand(){
 			...
 			if ($this->produk_model->deldata('brand',$where)) {
 				$notif = "<div class='alert bg-success'><i class='fa fa-alert'></i>Data berhasil di hapus</div>";
 				$this->session->set_flashdata('notif',$notif);
 				redirect('home/showbrand');
 			}
 		...
 		public function deltrans(){
 			...
 			if ($this->produk_model->deldata('transaksi',$where)) {
 				$notif = "<div class='alert bg-success'><i class='fa fa-alert'></i>Data berhasil di hapus</div>";
 				$this->session->set_flashdata('notif',$notif);
 				redirect('home/showtransaksi');
 			}
